...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim):
        ...
        self.U_q = nn.Linear(in_dim, hidden_dim)
        self.U_v = nn.Linear(in_dim, hidden_dim)
        self.U_k = nn.Linear(in_dim, hidden_dim)
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Y (torch.Tensor): shape B x N x D
        """
        # Shape B x N x K
        q = self.U_q(X)
        k = self.U_k(X)
        v = self.U_v(X)
        # Shape is B x N x N
        A = self.softmax(torch.einsum('bij, bkj -> bik', q,k)/ torch.sqrt(self.hidden_dim))
...
class MultiHeadedAttentionBlock(nn.Module):
    def __init__(self, in_dim: int, hidden_dim:int, num_heads:int):
        super().__init__()
        self.H = num_heads
        self.heads = [AttentionBlock(in_dim, hidden_dim) for i in range(num_heads)]
...
if __name__ == "__main__":
    ...
    multi_attention_block = MultiHeadedAttentionBlock(in_dim, hidden_dim, num_heads)
